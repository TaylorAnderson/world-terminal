  ...
  */
  ...
  void UpdateMesh(float myTime) { //set the data for the endmesh
    ...
    for (int i = 0, iL = hyphenedText.Length; i < iL; i++) {
      ...
      if (CurrentTextInfo.jitterData != null && !data.disableAnimatedText && !disableAnimatedText) { //just dont jitter if animating text is overridden
        areWeAnimating = true;
      ...
      if ((VerticalLimitStored > 0f && verticalLimitMode != VerticalLimitMode.Ignore) &&  //if vertical limit is on and not set to ignore...
          (CurrentTextInfo.pos.y + CurrentTextInfo.size + UpdateMesh_lowestLineOffset.y - anchorOffset.y > -rawTopLeftBounds.y + 0.00001f/* || info[i].pos.y < -rawBottomRightBounds.y*/)) { //hide all text that extends beyond the vertical limit
      ...
      //Color data:
      if (CurrentTextInfo.isQuad && !CurrentTextInfo.quadData.silhouette) { //if it's a quad but not a silhouette
      ...
      else if (CurrentTextInfo.gradientData != null) { //gradient speed + gradient spread
      ...
      else if (CurrentTextInfo.textureData != null) { //it has a texture
        endCol32[4 * i + 0] = Color.white; //set color to be white, so it doesn't interfere with texture
        endCol32[4 * i + 1] = Color.white;
        endCol32[4 * i + 2] = Color.white;
        endCol32[4 * i + 3] = Color.white;
      }
      else if (CurrentTextInfo.colorData != null) { //use colordata
